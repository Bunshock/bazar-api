...
public class SaleService implements ISaleService {
    ...
    private final SaleMapper serviceUtils;
    
    @Autowired
    public SaleService(ISaleRepository ventaRepository,
            IProductRepository productoRepository, IClientRepository clienteRepository,
            IUserRepository userRepository, SaleMapper serviceUtils) {
    ...
    @Override
    @Transactional
    public void saveVenta(InputSaleDTO datosVenta, Long id_cliente) {
        Sale venta = new Sale();
        ...
        List<Product> listaProductos = datosVenta.getListaIdProductos().stream()
        ...
        Client cliente = clienteRepository.findById(id_cliente)
    ...
    @Override
    public List<ShowSaleDTO> getVentas() {
        List<Sale> listaVentas = ventaRepository.findAll();
        
        return listaVentas.stream()
                .map(venta -> serviceUtils.SaleToShowSaleDTO(venta))
                .collect(Collectors.toList());
    }

    @Override
    public ShowSaleDTO getVentaById(Long codigo_venta) {
        Sale venta = ventaRepository.findById(codigo_venta)
                .orElseThrow(() -> new EntityNotFoundException("La venta con "
                        + "id (" + codigo_venta + ") no fue encontrada"));
        
        return serviceUtils.SaleToShowSaleDTO(venta);
    }
    ...
    @Override
    @Transactional
    public ShowSaleDTO editVenta(Long codigo_venta, InputSaleDTO ventaEditada, Long id_cliente) {
        Sale venta = ventaRepository.findById(codigo_venta)
                .orElseThrow(() -> new EntityNotFoundException("La venta con "
                        + "id (" + codigo_venta + ") no fue encontrada"));
        ...
        if (ventaEditada.getListaIdProductos() != null) {
            List<Product> listaProductos = productoRepository
    ...
    @Override
    public List<Product> getVentaProductos(Long codigo_venta) {
        Sale venta = ventaRepository.findById(codigo_venta)
                .orElseThrow(() -> new EntityNotFoundException("La venta con "
                        + "id (" + codigo_venta + ") no fue encontrada"));
    ...
    @Override
    public DateSalesSummaryDTO getVentaResumeByDate(LocalDate fecha) { 
        ...
        return new DateSalesSummaryDTO(montoTotal, cantidadVentas);
    }

    @Override
    public Sale getHighestTotalVenta() {
    ...
    public void concretarVenta(Long codigo_venta) {
        ...
        cantidadesPorId.entrySet().stream()
                .map(entry -> {
                    ...
                    Product producto = productoRepository.findById(codigoProducto)
                            .orElseThrow(() -> new EntityNotFoundException("No se"
    ...
    @Override
    @Transactional
    public void saveMiVenta(InputSaleDTO datosVenta) {
    ...
    @Override
    public List<ShowSaleDTO> getMisVentas() {
        // Obtenemos el usuario relacionado al cliente logueado
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        UserEntity user = userRepository.findByUsername(username)
                .orElseThrow(() -> new UsernameNotFoundException("El usuario con "
        ...
        List<Sale> listaMisVentas = ventaRepository.findByUnCliente_IdCliente(
    ...
    @Override
    public ShowSaleDTO getMiVentaById(Long codigo_venta) {
        // Obtenemos el usuario relacionado al cliente logueado
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        UserEntity user = userRepository.findByUsername(username)
                .orElseThrow(() -> new UsernameNotFoundException("El usuario con "
        ...
        Sale venta = ventaRepository.findByUnCliente_IdClienteAndCodigoVenta(
